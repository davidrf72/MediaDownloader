...
from Utils.Tools.task_manager import TaskManager
from Utils.Frames.link_list_frame import LinkListFrame
from Utils.Frames.title_frame import TitleFrame
...
class MainWindow(ctk.CTk):
    ...
    def __build_window(self):
        ...
        self.btn_title_close_info = ctk.CTkButton(self.frm_title, text='⏪⏩', text_color='black', font=font, command=self.__to_change_size)
        ...
        self.frm_link = ttk.LabelFrame(self.frm_label, labelanchor=NW, border=0, text='')#, text='Source link')
        self.frm_link.place(relx=0.05, rely=0.3, relwidth=0.9, relheight=0.17)
        frm_source_link = ctk.CTkFrame(self.frm_link, fg_color='blue')
        self.frm_link.configure(labelwidget=frm_source_link)
        lbl_source_link = ctk.CTkLabel(frm_source_link, text='Source link...', fg_color='blue', font=font, text_color='lightgreen')
        lbl_source_link.configure(corner_radius=0, anchor='w')
        lbl_source_link.pack(fill=Y, side=LEFT)
        self.btn_source_link = ctk.CTkButton(frm_source_link, text='Add list urls', font=('Times', 14, 'bold'), text_color='lightgreen')
        self.btn_source_link.configure(corner_radius=20, hover_color='red', text_color='black', command=self.__url_list_frame)
        self.btn_source_link.pack(fill=Y, side=LEFT, padx=(140, 0))
        font = ctk.CTkFont(family='Times', size=12, weight='bold')
    ...
    def __download(self, link_to_download: str):
        ...
        if not link_to_download.lower().startswith(r'https://www.youtube.com/') and\
            ...
            if link_to_download.lower().startswith(r'https://www.facebook.com/') or link_to_download.lower().startswith(r'https://fb.watch/'):
                ...
                #Thread(target=fb_downloader.start, args=(self.txt_to_link.get(), self.__saved_file_path, self.__cur_logging), daemon=True).start()
                pass
            else:
                self.__y_thread_count += 1
                downloader = Downloader(parent_window=self, call_back=self.__config_on_start, progress_handler=self.__add_data(link_to_download),
                                        icon=self.__cur_path.joinpath('social_youtube_2756.ico'), resolution=self.__cur_resolution)
                ...
                self.__task_manager.append_task(task=task)
                #Thread(target=downloader.start, args=(self.txt_to_link.get(), self.__saved_file_path, self.__cur_resolution, audio_only, self.__cur_logging), daemon=True).start()
        pass
